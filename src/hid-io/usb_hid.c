                         ...
                         uint8_t **data)
{
    ...
    case ZMK_HID_REPORT_ID__IO_PLOVER_HID:
    {
        struct zmk_hid_plover_hid_report_alt *report = zmk_hid_get_plover_hid_report_alt();
        *data = (uint8_t *)report;
        *len = sizeof(*report);
        break;
    }
...
#if IS_ENABLED(CONFIG_ZMK_HID_IO_PLOVER_HID)
int zmk_usb_hid_send_plover_hid_report_alt()
{
    struct zmk_hid_plover_hid_report_alt *report = zmk_hid_get_plover_hid_report_alt();
    return zmk_usb_hid_send_report_alt((uint8_t *)report, sizeof(*report));
}
